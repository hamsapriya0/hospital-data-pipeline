def build_gold():
    ...
    run_sql("""
    CREATE TABLE IF NOT EXISTS gold.top_doctors_completed AS
    SELECT 
        d.doctor_id,
        d.name AS doctor_name,
    ...
    run_sql("""
    CREATE TABLE gold.top_disease_per_age_group AS
    ...
    FROM silver.diagnosis d
    JOIN silver.appointments a ON d.appointment_id = a.appointment_id
    JOIN silver.patients p ON a.patient_id = p.patient_id
    GROUP BY age_group, d.disease
)
SELECT age_group, disease, disease_count
FROM disease_counts
WHERE rn = 1
ORDER BY age_group;
